...
trait Genoapay
{
     ...
     * @param $I
     */
    protected function configGenoaPaySettings($I)
    {
        $I->cli(['option', 'set', 'woocommerce_store_address','29th-Street']);
        $I->cli(['option', 'set', 'woocommerce_store_address_2 "#343"']);
        $I->cli(['option', 'set', 'woocommerce_store_city', 'Auckland']);
        $I->cli(['option', 'set', 'woocommerce_default_country', 'NZ:AK']);
        $I->cli(['option', 'set', 'woocommerce_store_postcode', '0600']);
        $I->cli(['option', 'set', 'woocommerce_currency', 'NZD']);
        ...
        $shippingZone = $I->cliToString(['wc', 'shipping_zone', 'create', '--name=Newzeland','--user=admin',"--porcelain"]);
        $I->cli(['wc', 'shipping_zone_method', 'create', $shippingZone,'--user=admin','--method_id=flat_rate','--enabled=true',"--porcelain"]);
        $I->cli(['wc', 'payment_gateway', 'update', 'genoapay','--user=admin','--enabled=true',"--settings='".$settings."'","--porcelain"]);
        $I->cli(['cache', 'flush']);
    }
     ...
     * @param $I
     */
    protected function loginToGenoaPay($I)
    {
        $I->amOnUrl('https://app.uat.genoapay.com/logout');
        $I->wait(3);
        $I->amOnUrl('https://app.uat.genoapay.com/');
        $I->wait(3);
    ...
    protected function addProductToCart($I) {
        ...
        $I->see('Polo');
        $I->amOnPage('/checkout');
        $I->waitForElement('#payment_method_genoapay', 30);
    }
    ...
    protected function doCheckout($I) {
        ...
        $I->fillField(['name' => 'billing_city'], 'Auckland');
        ...
        $I->selectOption(['name' => 'billing_country'], 'NZ');
        $I->wait(3);
        $I->selectOption(['name' => 'billing_state'], 'AK');
        ...
        $uri = $I->grabFromCurrentUrl();
        if(strpos($uri,"/?wc-api=genoapay_return_action&token=") !== false) {
            $I->amOnPage($uri);
    ...
    protected function doFullRefund($I,$orderId=null)
    {
        ...
        $I->see('Payment was successful via genoapay');
