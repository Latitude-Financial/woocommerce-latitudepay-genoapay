...
WP_CONTAINER=${1-woocommerce_latitudepay_wordpress}
SITE_URL=${WP_URL-"woocommerce.localhost"}
...
cli wp plugin is-active woocommerce-latitudepay-genoapay > /dev/null
if [[ $? -eq 0 ]]; then
	set -e
	echo
	echo "WC Latitudepay Payment is installed and active"
...
cli wp core install \
	--path=/var/www/html \
	--url=$SITE_URL \
	--title=${SITE_TITLE-"WooCommerce Latitudepay Dev"} \
...
echo "Activating the WooCommerce Latitudepay Payment plugin..."
cli wp plugin activate woocommerce-latitudepay-genoapay
